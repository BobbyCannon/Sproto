...
namespace Sproto.Nmea
{
	public abstract class RmcMessage : NmeaMessage
	{
		...
		protected RmcMessage(NmeaMessageType type) : base(type)
		{
		}
		...
		{
			var result = $"{Type} Time:{TimeOfFix} Warning:{NavigationReceiverWarning} Latitude:{Latitude} Longitude:{Longitude} Speed:{SpeedOverGround} Course:{CourseMadeGood} Date:{DateOfFix} Variation:{MagneticVariation} Mode:{ModeIndicator} ";
