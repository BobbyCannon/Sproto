...
namespace OSC.Tests.OSC
{
	...
	public class OscCommandTests
	{
		...
		{
			var message = new OscMessage(TestCommand.Command, (ulong) 23, "John", 20, new DateTime(2000, 01, 15), 5.11f, 164.32,
		...
		public void ToFromString()
		{
			...
			actual.Escape().Dump();
			Assert.AreEqual("/test,23U,\"John\",20,{ Time: 2000-01-15T00:00:00.0000Z },5.11f,164.32d,\'\u0004\',{ Blob: 0x000101020305080D },True,\"e3966202-40fa-443d-b21f-e1528a1e6dfe\",4294967295u,9223372036854775807L", actual);
		...
		public class TestCommand : OscCommand
		{
			...
			{
				OscMessage = new OscMessage(Time, Command, Id, Name, Age, BirthDate, Height, Weight, 
					Rating, Values, Enable, SyncId, Visits, VoteId);
			}
