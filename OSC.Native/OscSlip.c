...
OscError OscSlipEncodePacket(const OscPacket* const oscPacket, size_t* const slipPacketSize, char* const destination, const size_t destinationSize)
{
	*slipPacketSize = 0; // size will be 0 if function unsuccessful
	...
	for (unsigned int packetIndex = 0; packetIndex < oscPacket->size; packetIndex++)
	{
...
/*
 * @brief Initializes an OSC SLIP decoder structure.
 *
 * An OSC SLIP decoder structure must be initialized before use.  A
 * ProcessPacket function must be implemented within the application and
 * assigned to the OSC SLIP decoder structure after initialization.
 *
 * Example use:
 * @code
 ...
 *     OscSlipDecoderInitialize(&oscSlipDecoder);
