...
/*
 * @brief Initializes an OSC bundle with a specified OSC time tag.
 *
 * An OSC bundle must be initialized before use.  The oscTimeTag argument may be
 ...
 * tag value is intended to be overwritten after initialization of the OSC
 * bundle.
 *
 * Example use:
 * @code
 * OscBundle oscBundle;
 * OscBundleInitialize(&oscBundle, oscTimeTagZero);
 * oscBundle.oscTimeTag.value = 0x100000000; // overwrite OSC time tag with value of 1 second
 * @endcode
 *
 * @param oscBundle OSC bundle to be initialized.
 * @param oscTimeTag OSC time tag.
 */
void OscBundleInitialize(OscBundle* const oscBundle, const OscTimeTag oscTimeTag)
{
 ...
 * @param oscTimeTag OSC time tag.
 */
void OscBundleInitializeExtended(OscBundle* const oscBundle, const OscTimeTag oscTimeTag)
{
	OscBundleInitialize(oscBundle, oscTimeTag);
	oscBundle->header[0] = '+';
}
 ...
 *
 * The oscContents argument must point to an initialized OSC message or OSC
 ...
 * OscMessageInitialize(&oscMessageToAdd, "/example/address/pattern");
 *
 * OscBundle oscBundleToAdd;
 * OscBundleInitialize(&oscBundleToAdd, oscTimeTagZero);
 *
 * OscBundle oscBundle;
 * OscBundleInitialize(&oscBundle, oscTimeTagZero);
...
OscError OscBundleAddContents(OscBundle* const oscBundle, const void* const oscContents)
{
	...
	OscError oscError = OscErrorInvalidContents; // error: invalid or uninitialized OSC contents
	if (OscContentsIsMessage(oscContents) == true)
	{
		...
		oscError = OscMessageToCharArray((OscMessage *) oscContents, &oscBundleElementSize, oscBundleElement.contents,
			OscBundleGetRemainingCapacity(oscBundle));
		oscBundleElement.size.int32 = (int32_t) oscBundleElementSize;
	}
	if (OscContentsIsBundle(oscContents) == true)
	{
		size_t oscBundleElementSize;
		oscError = OscBundleToCharArray((OscBundle *) oscContents, &oscBundleElementSize, oscBundleElement.contents,
			OscBundleGetRemainingCapacity(oscBundle));
		oscBundleElement.size.int32 = (int32_t) oscBundleElementSize;
	}
	if (oscError != 0)
...
{
	const size_t remainingCapacity = MAX_OSC_BUNDLE_ELEMENTS_SIZE - oscBundle->oscBundleElementsSize - sizeof(
		OscArgument32); // account for int32 size required by OSC bundle element
...
{
	// size will be 0 if function unsuccessful
	*oscBundleSize = 0; 

	bool isExtended = oscBundle->header[0] == '+';
	uint8_t crcSize = isExtended ? 4 : 0;

	if ((sizeof (OSC_BUNDLE_HEADER) + sizeof(OscTimeTag) + oscBundle->oscBundleElementsSize + crcSize) > destinationSize)
	{
	...
	if (isExtended)
	{
		//((uint16_t) oscPacket->contents[oscPacket->size - 3] * 256) + oscPacket->contents[oscPacket->size - 4];

		// Calculate CRC and populate it.
		uint16_t crc = OscCalculateCrc16((uint8_t *) &destination[0], destinationIndex);
		destination[destinationIndex++] = (char) crc;
		destination[destinationIndex++] = (char) (crc >> 8);
		destinationIndex += 2;
	}
...
/*
 * @brief Initializes an OSC bundle from a byte array contained within an OSC
 * packet or containing OSC bundle.
 *
 * This function is used internally and should not be used by the user
 * application.
 ...
 * @return Error code (0 if successful).
 */
OscError OscBundleInitializeFromCharArray(OscBundle* const oscBundle, const char* const source,
