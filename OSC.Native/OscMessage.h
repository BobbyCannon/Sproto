...
typedef struct
{
	char oscAddressPattern[MAX_OSC_ADDRESS_PATTERN_LENGTH + 1];
	// must be first member so that first byte of structure is equal to '/'.  Null terminated.
...
OscError OscMessageInitializeFromCharArray(OscMessage* const oscMessage, const char* const source, const size_t size);
