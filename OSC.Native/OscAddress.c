 ...
 * OscMessageInitialize(&oscMessage, "/example/oscAddress/pattern");
	...
	while (*oscAddressPattern != '\0')
	{
		if (*oscAddress == '\0')
		{
			...
			else
			{
				...
				// handle trailing 'zero character' expressions
			}
...
static bool MatchStar(const char* * const oscAddressPattern, const char* * const oscAddress, const bool isPartial)
{
	...
	do
	{
		...
		if (MatchExpression(oscAddressPattern, oscAddress, isPartial) == true)
		{
 ...
 * @return True if OSC address pattern and target OSC address match.
 */
static bool MatchCurlyBraces(const char* * const oscAddressPattern, const char* * const oscAddress,
 ...
 */
OscError OscAddressGetPartAtIndex(const char* oscAddressPattern, const unsigned int index, char* const destination,
