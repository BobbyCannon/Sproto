...
/*
 * @brief Minimum size (number of bytes) of an OSC bundle as per the OSC specification.
 */
...
/*
 * @brief Maximum combined size (number of bytes) of all OSC bundle elements that may be contained within an OSC bundle.
 */
#define MAX_OSC_BUNDLE_ELEMENTS_SIZE (MAX_OSC_BUNDLE_SIZE - sizeof(OSC_BUNDLE_HEADER) - sizeof(OscTimeTag) - 2)
...
typedef struct
{
	char header[sizeof(OSC_BUNDLE_HEADER)];
	// must be the first member so that the first byte of structure is equal to '#'.
...
OscError OscBundleInitializeFromCharArray(OscBundle* const oscBundle, const char* const source, const size_t numberOfBytes);
