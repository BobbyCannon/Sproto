...
/*
 * @brief Initializes an OSC message.
 *
 * An OSC message must be initialized before use.  The oscAddressPattern
 * argument must be a null terminated string of zero of more characters.  A
 * message may be initialized without an address pattern by parsing an
 * oscAddressPattern value of "".  This may be of use if the address pattern is
 * undetermined at the time of initialization.  In which case, the address
 * pattern may be set later using OscMessageSetAddressPattern.
 *
 * Example use:
 * @code
 * OscMessage oscMessage;
 * OscMessageInitialize(&oscMessage, "/example/address/pattern");
 * @endcode
 *
 * @param oscMessage OSC message to be initialized.
 * @param oscAddressPattern OSC address pattern as null terminated string.
 * @return Error code (0 if successful).
 */
OscError OscMessageInitialize(OscMessage* const oscMessage, const char* oscAddressPattern)
{
 ...
 * @code
 * OscMessage oscMessage;
 * OscMessageInitialize(&oscMessage, "");
...
OscError OscMessageAddBlob(OscMessage* const oscMessage, const char* const source, const size_t numberOfBytes)
{
	...
	for (unsigned int sourceIndex = 0; sourceIndex < numberOfBytes; sourceIndex++)
	{
...
/*
 * @brief Initializes an OSC message from a byte array contained within an OSC
 ...
 * @return Error code (0 if successful).
 */
OscError OscMessageInitializeFromCharArray(OscMessage* const oscMessage, const char* const source, const size_t numberOfBytes)
{
	OscMessageInitialize(oscMessage, "");
	...
	oscMessage->oscTypeTagString[oscMessage->oscTypeTagStringLength] = '\0'; // null terminate string
	oscMessage->argumentCount = oscMessage->oscTypeTagStringLength - 1; // Remove the comma
 ...
 * @return Error code (0 if successful).
 */
OscError OscMessageGetArgumentAsString(OscMessage* const oscMessage, char* const destination,
	const size_t destinationSize)
{
	...
	switch (OscMessageGetArgumentType(oscMessage))
	{
		...
		case OscTypeTagBlob:
		{
			...
			if (destination[blobSize - 1] != '\0')
			{
				// if blob not null terminated
...
OscError OscMessageGetArgumentAsMidiMessage(OscMessage* const oscMessage, MidiMessage* const midiMessage)
{
	...
	switch (OscMessageGetArgumentType(oscMessage))
	{
		...
		{
			size_t blobSize;
			const OscError oscError = OscMessageGetBlob(oscMessage, &blobSize, (char *) &midiMessage,
				sizeof(MidiMessage));
			if (oscError != 0)
			{
				return oscError;
